 ...
 * 2/22/2022
 * description:
 * This is the client which sends the initial array to the server for calculation of median. These two programs together make a simple REST API
 * where the client sends values, and the server responds with the result for the client to use. 
 *
 * Usage: run ./ravg 127.0.0.1 [array of numbers spaced excluding the brackets] AFTER the server has started. Will receive response by server
 * After this is ran. 
 */
