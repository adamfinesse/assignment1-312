 ...
 * next process in line runs, finally all ending then detaching the memory locations. Similar to q1 but uses forks to utilize the shared memory 
 * of the parent and children rather than using a key to make a new one.
 *
 * Usage: run server after compilation and it will print out the processes in order.
 */
